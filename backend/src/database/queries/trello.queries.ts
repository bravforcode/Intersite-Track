...
export async function saveCardMapping(
  ...
  const payload = {
    ...
    trello_card_url: trelloCardUrl ?? undefined,
    created_at: existingData?.created_at ?? nowIso(),
    updated_at: nowIso(),
  };
  await ref.set(payload, { merge: true });
...
export async function saveStatusMapping(
  ...
  const payload = {
    ...
    trello_list_name: trelloListName ?? undefined,
    created_at: existingData?.created_at ?? nowIso(),
  };
  await ref.set(payload, { merge: true });
...
export async function saveUserMapping(
  ...
  const payload = {
    ...
    trello_username: trelloUsername ?? undefined,
    created_at: existingData?.created_at ?? nowIso(),
  };
  await ref.set(payload, { merge: true });
...
export async function createSyncLog(
  ...
  const log: Partial<TrelloSyncLog> = {
    ...
    trello_card_id: data.trello_card_id ?? undefined,
    action: data.action,
    status: "pending",
    retry_count: 0,
    request_payload: data.request_payload ?? undefined,
    created_at: nowIso(),
  };
  await ref.set(log);
...
export async function updateSyncLog(
  ...
  const payload: Record<string, unknown> = {
    ...
    response_payload: data.response_payload ?? undefined,
  };
