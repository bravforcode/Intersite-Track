...
# Option 2: Apply Firestore rules/indexes directly
...
- [ ] **Step 4: Remove legacy database references**

Check `package.json`:

```bash
rg -n "legacy db|legacy auth sdk|legacy env var" package.json src server scripts docs .kiro
```

Remove imports from code:

```bash
rg -n "legacy adapter|legacy env var|deprecated auth client" src server
# Delete legacy DB adapter files if they still exist
rm -f server/config/legacy-db.ts src/lib/legacy-auth.ts server/config/database.ts
```
