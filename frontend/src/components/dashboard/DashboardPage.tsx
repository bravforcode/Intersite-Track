...
export function DashboardPage({ user, onViewTask, onViewAll, refreshTrigger }: DashboardPageProps) {
  ...
  const slaBreached = activeTasks.filter((t) => t.sla_status === "breached").length;
  const slaWarning = activeTasks.filter((t) => t.sla_status === "warning").length;
  ...
  const focusCards = [
    ...
    { label: "SLA Warning", value: slaWarning, tone: "text-amber-600" },
    { label: "SLA Breached", value: slaBreached, tone: "text-rose-600" },
