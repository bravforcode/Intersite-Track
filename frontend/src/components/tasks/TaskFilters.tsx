...
export function TaskFilters({ users, onFilterChange }: TaskFiltersProps) {
  ...
  const clearFilters = () => {
    const next: FilterValues = { search: "", status: "", priority: "", assignee: "", dateFrom: "", dateTo: "", sla_status: "" };
    setFilters(next);
    onFilterChange(next);
    ...
    <div className="app-surface p-4 rounded-2xl space-y-4">
      ...
      {showFilters && (
        ...
        >
          ...
          <div>
            <label className="block text-xs font-bold uppercase app-soft mb-1">สถานะ SLA</label>
            <select
              className="w-full px-4 py-2 rounded-xl text-sm app-field"
              value={filters.sla_status}
              onChange={(e) => update({ sla_status: e.target.value })}
            >
              <option value="">ทั้งหมด</option>
              <option value="fine">ON TRACK</option>
