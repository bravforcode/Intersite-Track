...
import { 
  CalendarIcon, ExternalLinkIcon, 
  CheckCircle2Icon, AlertCircleIcon, PlusIcon,
...
export function ProjectDetailModal({ projectId, isOpen, onClose, onCreateTask }: ProjectDetailModalProps) {
  ...
  const [projectMessage, setProjectMessage] = useState<{ type: "error" | "success"; text: string } | null>(null);
  ...
  const handleAddMilestone = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProjectMessage({ type: "success", text: "บันทึก Milestone แล้ว" });
      fetchProjectDetails();
    } catch (err) {
      console.error(err);
      setProjectMessage({ type: "error", text: "ไม่สามารถบันทึก Milestone ได้" });
    }
  };
  ...
  const handleAddBlocker = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProjectMessage({ type: "success", text: "บันทึกปัญหาแล้ว" });
      fetchProjectDetails();
    } catch (err) {
      console.error(err);
      setProjectMessage({ type: "error", text: "ไม่สามารถบันทึกปัญหาได้" });
    }
  };
  ...
  const handleAddUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProjectMessage({ type: "success", text: "บันทึกอัปเดตรายสัปดาห์แล้ว" });
      fetchProjectDetails();
    } catch (err) {
      console.error(err);
      setProjectMessage({ type: "error", text: "ไม่สามารถบันทึกอัปเดตได้" });
    }
  };
  ...
  const handleResolveBlocker = async (blockerId: string) => {
    try {
      ...
      setProjectMessage({ type: "success", text: "ปลดล็อกปัญหาแล้ว" });
      await fetchProjectDetails();
    } catch (err) {
      console.error(err);
      setProjectMessage({ type: "error", text: "ไม่สามารถปลดล็อกปัญหาได้ กรุณาลองใหม่อีกครั้ง" });
    } finally {
        ...
        {projectMessage && (
          <div
            className={`mb-4 flex items-start justify-between gap-3 rounded-lg border px-4 py-3 text-sm ${
              projectMessage.type === "error"
                ? "border-red-200 bg-red-50 text-red-700"
                : "border-emerald-200 bg-emerald-50 text-emerald-700"
            }`}
            role={projectMessage.type === "error" ? "alert" : "status"}
          >
            <span>{projectMessage.text}</span>
            <button
              type="button"
              onClick={() => setProjectMessage(null)}
              className="rounded-lg px-2 font-bold opacity-70 hover:opacity-100"
          ...
          {activeTab === 'tasks' && (
            <div className="space-y-3">
              <div className="flex justify-between items-center mb-4">
                ...
                {onCreateTask && (
                  <Button variant="secondary" size="sm" onClick={() => { onClose(); onCreateTask(); }}>
                    <PlusIcon size={14} className="mr-1" /> เพิ่มงาน
                  </Button>
                )}
              </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {b.status === 'active' && (
                      <Button
                        size="sm"
                        variant="secondary"
                        loading={resolvingBlockerId === b.id}
